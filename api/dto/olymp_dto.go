...
type OlympQueryParams struct {
	Levels   []string `form:"level" example:"1,2"`     // Уровень олимпиады (можно передавать несколько)
	Profiles []string `form:"profile" example:"math"`  // Профиль олимпиады
	Search   string   `form:"search" example:"физика"` // Поиск по названию
...
type OlympiadShortResponse struct {
	...
	Name       string `json:"name" example:"Олимпиада по математике"` // Название олимпиады
	Level      int16  `json:"level" example:"1"`                      // Уровень олимпиады
	Profile    string `json:"profile" example:"math"`                 // Профиль олимпиады
